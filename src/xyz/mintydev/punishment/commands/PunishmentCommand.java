...
public class PunishmentCommand extends Command {
	...
	public boolean execute(CommandSender sender, String[] args, String label) throws Exception {
		...
		for (final String arg : args) {
			...
			if (totalDuration > 0 && arg.equalsIgnoreCase(args[1]) && takeTime)
				continue;
			reason += arg + " ";
		}
		...
		Punishment pt = new Punishment(ptype, playerUUID, playerName, isPlayer ? ((Player) sender).getUniqueId() : null,
				sender.getName(), now, (totalDuration > 0 ? new Date(now.getTime() + totalDuration) : null), reason);

		pt.execute(silent);

		final String applyMsg = pt.replaceWithValues(
				LangManager.getMessage(totalDuration > 0 ? this.tempType.getLangPath() + ".apply-message"
						: this.type.getLangPath() + ".apply-message"));
	...
	public void wrongUsage(CommandSender sender, String label) {
		String res = String.format("§cUsage: /%s %s", label,
				"<player>" + (takeTime ? " [time]" : "") + " [reason] [-s]");
